...
#
### CHECK DEPENDENCIES
$Command0 = {& "$ROOT\core\upkeep\jsonValidation.ps1" $ROOT $JLOG}
Invoke-Command -ScriptBlock $Command0

### AUTO-UPDATE
$Command1 = {& "$ROOT\core\upkeep\syncScripts.ps1" $ROOT $JLOG}
...
{
    Write-Output((Get-Date -Format G) + " ERROR Installing by InstallerMan") | Tee-Object -FilePath $JLOG -Append -Verbose
    Write-Host -NoNewLine 'ERROR Installing by InstallerMan';
    $null = $Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown');
    throw "ERROR installing by InstallerMan"
}
$Reader = "$ROOT\conf\get\reader.ps1"
$Comm001 = {& $Reader "Jenkins" "path" }
$JENKINS_HOME = Invoke-Command -ScriptBlock $Comm001
$Comm002 = {& $Reader "Jenkins" "exists" }
...
if($isAlreadyInstalled.tolower() -eq "false")
{
    ...
    $Command3 = {& "$ROOT\core\upkeep\JenkinsManager.ps1" $ROOT $JLOG}
