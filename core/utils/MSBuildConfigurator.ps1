    ...
    if(Test-Path -Path $filePath)
    {
        ...
        if(Test-Path -Path $hudsonFilePath)
        {
            ...
            if(-not($existsMyMSBuild))
            {
                ...
                $noRet1 = $xmlfile.ssonhhud.Installations["hhudson"].AppendChild($newSubItem1)
                $newSubItem2 = $xmlfile.CreateElement("home")
                $newSubItem2.PsBase.InnerText = $MSBUILD_PATH
                $noRet2 = $xmlfile.ssonhhud.Installations["hhudson"].AppendChild($newSubItem2)
                $newSubItem3 = $xmlfile.CreateElement("properties")
                #$newSubItem3.PsBase.InnerText = $
                $noRet3 = $xmlfile.ssonhhud.Installations["hhudson"].AppendChild($newSubItem3)
                $xmlfile.Save($hudsonFilePath)
            }
        ...
        else
        {
            ...
            $newSubItem1 = $xmlfile.CreateElement("name")
            $newSubItem1.PsBase.InnerText = $MSBUILD_NAME
            $noRet1 = $xmlfile.hudson.Installations["hud2son"].AppendChild($newSubItem1)
            $newSubItem2 = $xmlfile.CreateElement("home")
            $newSubItem2.PsBase.InnerText = $MSBUILD_PATH
            $noRet2 = $xmlfile.hudson.Installations["hud2son"].AppendChild($newSubItem2)
            $newSubItem3 = $xmlfile.CreateElement("properties")
            #$newSubItem3.PsBase.InnerText = $
            $noRet3 = $xmlfile.hudson.Installations["hud2son"].AppendChild($newSubItem3)
            $xmlfile.Save($hudsonFilePath)
